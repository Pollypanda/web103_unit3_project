...
//                 <div 
//                     key={location.id} 
//                     onMouseOver={() => handleMouseOver(location.name)} 
//                     onMouseLeave={handleMouseLeave}
//                     className={`venue-button-overlay ${hoveredVenue === location.name ? 'visible' : ''}`}
//                 >
...
//                 {locations.map((location, index) => (
//                     <a key={location.id} href={`/${location.route}`}>
//                         <polygon id={`venue${index + 1}`} points={location.points} />
//                     </a>
//                 ))}
//             </svg>
//         </div>
...
import React, { useState, useEffect } from 'react';
import { fetchLocationByIndex } from '../services/LocationsAPI.jsx';
import unitygrid from '../assets/unitygrid.jpg';
import '../css/Locations.css';
...
const Locations = () => {
    const [locations, setLocations] = useState([]);
    const [error, setError] = useState(null);
    const someIndexLimit = 10; // Define your limit here

    useEffect(() => {
        const loadLocations = async () => {
            try {
                const fetchedLocations = [];
                for (let i = 0; i < someIndexLimit; i++) {
                    const location = await fetchLocationByIndex(i);
